...
interface TransactionOptionsModalProps {
  ...
  onSelectEdit: (tx: Transaction) => void;
  onSelectDelete: (tx: Transaction) => void;
}
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-full max-w-[390px] rounded-[24px] bg-[var(--color-surface)] p-6 shadow-[var(--shadow-float)] border-[var(--color-divider)]" showCloseButton={false}>
        <DialogHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          <DialogTitle className="text-[20px] font-semibold text-[var(--color-text)]">Opsi Transaksi</DialogTitle>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <span className="text-[14px] font-medium text-[var(--color-text)]">Edit Transaksi</span>
          </button>
          ...
          >
            ...
            <span className="text-[14px] font-medium text-[var(--color-negative)]">Hapus Transaksi</span>
          </button>
        </div>
      </DialogContent>
    </Dialog>
